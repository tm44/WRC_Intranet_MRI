...
{
    public class Block5PositivePayFormatter : BasePositivePayFormatter
    {
        public override string BankID => "900112";
        ...
        public Block5PositivePayFormatter(StreamWriter writer) : base(writer)
        {
        }
        ...
        public override void WriteCheckRow(CheckInfo checkInfo, bool includePayee)
        {
            ...
            //s.Append(checkInfo.TransactionCode); // Per Jaiya and Mary, we should not include this, as it adds an extra "O" that is causing problems
            s.Append(checkInfo.CheckNumber.PadLeft(10, '0')); // 10
            s.Append(string.Format("{0:f2}", checkInfo.CheckAmount).PadLeft(13, '0')); // changed to 13 after 9/7/22 email from Jaiya
            ...
            if (includePayee)
                ...
                s.Append(string.Empty.PadLeft(256, ' ')); // 256
